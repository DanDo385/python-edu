...
1. TYPE CHECKING AT RUNTIME
   ...
   Python:
       x + y    # Runtime: Check if x and y support +
...
- Others: Types required (checked at compile time)
...
1. add_numbers(a, b)
   ...
   - Works with int and float without type declarations
...
3. describe_type(value)
   - Type introspection using type() and isinstance()
...
def safe_divide(a: Union[int, float], b: Union[int, float]) -> float:
    ...
    PARAMETERS:
        a: Union[int, float] - Numerator (must be number)
        b: Union[int, float] - Denominator (must be number, cannot be zero)
...
def process_data(data: Union[int, str, list]) -> Any:
    ...
    PARAMETERS:
        data: Union[int, str, list] - Data to process
